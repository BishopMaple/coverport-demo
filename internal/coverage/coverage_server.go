...
func init() {
	// Only start the coverage server if the binary was built with -cover.
	// coverage.WriteMeta() returns an error when the binary lacks coverage
	// instrumentation, so we use it as a cheap detection mechanism.
	// This ensures production builds (without -cover) never open port 9095.
	var probe bytes.Buffer
	if err := coverage.WriteMeta(&probe); err != nil {
		// Binary was NOT built with -cover — do nothing
		return
	}
